...
static int epdc_pwr0_disable(struct regulator_dev *reg)
{
	...
	cur_reg_val = tps65180_current_Enable_Register & 0x3f;
	fld_mask =  BITFMASK(V3P3_SW_EN);
	fld_val = BITFVAL(V3P3_SW_EN, false);
	new_reg_val = tps65180_current_Enable_Register = to_reg_val(cur_reg_val, fld_mask, fld_val);
	tps6518x_reg_write(REG_TPS65180_ENABLE, new_reg_val);
					...
					int minuV, int uV, unsigned *selector)
{
	...
	if (uV < 200000)
		return 0;
#endif
//	printk("tps6518x_vcom_set_voltage uV = %d | minuV = %d\n", uV, minuV);
	...
	switch (tps6518x->revID & 15)
	{
		...
		case 6 : /* TPS65186 */
			...
			new_reg_val = vcom2_uV_to_rs(uV) & 255;
			retval = tps6518x_reg_write(REG_TPS65185_VCOM1,
					new_reg_val);
//			printk("tps6518x_vcom_set_voltage REG_TPS65185_VCOM1 = 0x%x\n", new_reg_val);
			...
			retval = tps6518x_reg_write(REG_TPS65185_VCOM2,
					new_reg_val);
//			printk("tps6518x_vcom_set_voltage REG_TPS65185_VCOM2 = 0x%x\n", new_reg_val);
			break;
		default :
		retval = -1;
	}
...
//	printk("tps6518x_vcom_get_voltage = %d\n", vcomValue);
	return vcomValue;
...
static int tps6518x_display_enable(struct regulator_dev *reg)
{
	...
	else
	{
		...
		cur_reg_val = tps65180_current_Enable_Register & 0x3f;
		fld_mask = BITFMASK(VDDH_EN) | BITFMASK(VPOS_EN) | BITFMASK(V3P3_SW_EN) |
			BITFMASK(VEE_EN) | BITFMASK(VNEG_EN);
		fld_val = BITFVAL(VDDH_EN, true) | BITFVAL(VPOS_EN, true) | BITFVAL(V3P3_SW_EN, true) |
...
static int tps6518x_regulator_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "tps6518x_regulator_probe starting\n");
	...
	dev_dbg(&pdev->dev, "tps6518x_pass_num = %d\n", tps6518x_pass_num);
	dev_dbg(&pdev->dev, "tps6518x_vcom = %d\n", tps6518x_vcom);
	...
	dev_dbg(&pdev->dev, "tps6518x_regulator_probe success\n");
	return 0;
err:
	while (--i >= 0)
